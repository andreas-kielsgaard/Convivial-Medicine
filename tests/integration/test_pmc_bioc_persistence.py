...
def test_fixture_mode_bioc_persistence_creates_source_and_manifest_rows(
    tmp_path: Path,
) -> None:
    ...
    expected_result = process_bioc_response_bytes(
        raw_bytes=FIXTURE_PATH.read_bytes(),
        artifact_store=LocalArtifactStore(tmp_path / "expected-bioc-artifacts"),
        request=request,
        http_status=200,
    ...
    result = runner.invoke(
        ...
        [
            ...
            str(FIXTURE_PATH),
            "--artifact-root",
            str(tmp_path / "cli-bioc-artifacts"),
            "--persist-db",
        ],
    )
